...
::note
If a layer is essential to your project, you can mark it as `required`. When a required layer cannot be resolved or installed (for example, if you forgot to clone or install it), Nuxt will throw an error instead of silently continuing the build.

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  extends: [
    ['github:username/repoName', { required: true }],
  ],
})
```
::
